"""
dataset condensation using Pseudo Outlier Exposure

using mixed corrupted images as outlier data
...
from utils.gen_ood_loader_cifar import gen_ood_data_cifar10, gen_ood_data_cifar100
...
def main():
    ...
    parser.add_argument('--save_path', type=str, default='result-poe', help='path to save results')
    ...
    num_train = len(dst_train)
    print('num_train:', num_train)
    model_eval_pool = get_eval_pool(args.eval_mode, args.model, args.model)

    # gen OE dataset
    args.type_list = ['jigsaw', 'invert', 'pixelate', 'gauss-speckle']
    num_oe_source = len(args.type_list)
    print('num_oe_source:', num_oe_source)
    ood_loader_list = []
    for i in range(num_oe_source):
        if args.dataset == 'CIFAR10':
            ood_data_, ood_loader_ = gen_ood_data_cifar10(args, args.type_list[i])
        elif args.dataset == 'CIFAR100':
            ood_data_, ood_loader_ = gen_ood_data_cifar100(args, args.type_list[i])
        else:
            print('invalid dataset!')
            exit()
        ood_loader_list.append(ood_loader_)
    ...
    for exp in range(args.num_exp):
        ...
        images_oe_all_list, labels_oe_all_list = [], []
        #indices_class_oe_list = []
        for i in range(num_oe_source):
            ood_loader = ood_loader_list[i]
            data_iter = iter(ood_loader)
            images_oe_all, labels_oe_all = data_iter.next()
            images_oe_all, labels_oe_all = images_oe_all.to(args.device), labels_oe_all.to(args.device)
            indices_class_oe = [[] for c in range(num_classes)]
            for i, lab in enumerate(labels_oe_all):
                lab = int(lab)
                indices_class_oe[lab].append(i)
            # list append
            images_oe_all_list.append(images_oe_all)
            labels_oe_all_list.append(labels_oe_all)
            #indices_class_oe_list.append(indices_class_oe)

        def get_images_oe(c, n):   # get random n images from class c
            idx = int(c % num_oe_source)
            images_oe_all_ = images_oe_all_list[idx]
            # idx_shuffle_ = np.random.permutation(indices_class_oe_[c])[:n]
            idx_shuffle_ = np.random.permutation(num_train)[:n]
            return images_oe_all_[idx_shuffle_]
